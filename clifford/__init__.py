...
class ArrayCounts:
    def __init__(self, size: int, padding: float):
        self.size: int = size
        self.count_array: np.ndarray = np.zeros([size, size], dtype=int)
        self.dx: np.ndarray = np.zeros([size, size], dtype=float)
        self.dy: np.ndarray = np.zeros([size, size], dtype=float)
        self.padding: float = padding


def update_counts(x0: float, y0: float, x: np.ndarray, y: np.ndarray, arr: ArrayCounts):
    lpad = arr.padding * arr.size
    scale = arr.size * (1 - 2 * arr.padding) / 2

    x_px = np.array((x + 1) * scale + lpad, dtype=int)
    y_px = np.array((y + 1) * scale + lpad, dtype=int)
