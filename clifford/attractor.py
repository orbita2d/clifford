...
@jit
def increment_rhea(x: float, y: float, a: float, b: float, c: float, d: float):
    """ Take a single point, increment it following the clifford attractor equations."""
    # Rescale so that attractor fits in [-1, 1]
    x_width = 1 + np.abs(c)
    y_width = 1 + np.abs(d)
    xn = (np.sin(a*y*y*y_width) + c * np.cos(a*x*x_width)) / x_width
    yn = (np.sin(b*x*x*x_width) + d * np.cos(b*y*y_width)) / y_width
    return xn, yn


@jit
def build_rhea(x: np.ndarray, y: np.ndarray, x0: float, y0: float, a: float, b: float, c: float, d: float, count: int):
    """Iterate through many points, producing array of points on attractor"""
    x[0], y[0] = increment_rhea(x0, y0, a, b, c, d)
    for i in range(1, count):
        x[i], y[i] = increment_rhea(x[i - 1], y[i - 1], a, b, c, d)


class AttractorSystem:
    """ ABC for attractor object."""
    def __init__(self, data: dict):
        self.type: str = data["type"]
        self.data: dict = data["data"]

    def increment(self, x: float, y: float, p: np.ndarray):
        raise SyntaxError('AttractorSystem is an abstract base class. increment() should be overwritten')

    def build(self, x: np.ndarray, y: np.ndarray, x0: float, y0: float, p: np.ndarray, count: int):
        raise SyntaxError('AttractorSystem is an abstract base class. build() should be overwritten')


class CliffordAttractor(AttractorSystem):
    def __init__(self, data):
        super(CliffordAttractor, self).__init__(data)

    def increment(self, x: float, y: float, p: np.ndarray):
        increment_clifford(x, y, p[0], p[1], p[2], p[3])

    def build(self, x: np.ndarray, y: np.ndarray, x0: float, y0: float, p: np.ndarray, count: int):
        build_clifford(x, y, x0, y0, p[0], p[1], p[2], p[3], count)


class RheaAttractor(AttractorSystem):
    def __init__(self, data):
        super(RheaAttractor, self).__init__(data)

    def increment(self, x: float, y: float, p: np.ndarray):
        increment_rhea(x, y, p[0], p[1], p[2], p[3])

    def build(self, x: np.ndarray, y: np.ndarray, x0: float, y0: float, p: np.ndarray, count: int):
        build_rhea(x, y, x0, y0, p[0], p[1], p[2], p[3], count)


def get_attractor(data: dict) -> AttractorSystem:
    if data["type"] == "clifford":
        return CliffordAttractor(data)
    elif data["type"] == "rhea":
        return RheaAttractor(data)
    else:
...
def test_closed(p: np.ndarray, attractor: AttractorSystem):
    ...
    attractor.build(x, y, x0, y0, p, count)
...
def get_frame(x0: float, y0: float, p: np.ndarray, count: int, arr: ArrayCounts, attractor: AttractorSystem):
